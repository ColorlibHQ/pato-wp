...
( function () {
	...
	var strings = window.patoScheme || { toDark: 'Switch to dark mode', toLight: 'Switch to light mode' };

	function isDark() {
		return root.classList.contains( 'pato-dark' );
	}
	...
	function apply( dark ) {
		root.classList.toggle( 'pato-dark', dark );
	...
	function init() {
		...
		// started.
		var buttons = document.querySelectorAll( '.pato-scheme-toggle .wp-block-button__link' );
		...
		try {
			if ( ! localStorage.getItem( KEY ) && window.matchMedia ) {
				window.matchMedia( '(prefers-color-scheme: dark)' ).addEventListener( 'change', function ( e ) {
					...
					root.classList.toggle( 'pato-dark', e.matches );
					Array.prototype.forEach.call( buttons, label );
				} );
			}
		} catch ( e ) {}
