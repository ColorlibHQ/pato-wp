 ...
 */
function pato_woocommerce_setup() {
...
add_action( 'after_setup_theme', 'pato_woocommerce_setup' );
 ...
 */
function pato_woocommerce_styles() {
	if ( ! class_exists( 'WooCommerce' ) ) {
		return;
	}
	 ...
	 */
	if ( ! apply_filters( 'pato_load_woocommerce_styles', $needed ) ) {
		return;
	}
...
add_action( 'wp_enqueue_scripts', 'pato_woocommerce_styles', 20 );
 ...
 * @return int
 */
function pato_woocommerce_columns() {
	return 3;
}
add_filter( 'loop_shop_columns', 'pato_woocommerce_columns' );
