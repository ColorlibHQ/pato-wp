 ...
 * call our filter.
 */
const PATO_UPDATE_HOST     = 'updates.colorlib.com';
const PATO_UPDATE_ENDPOINT = 'https://updates.colorlib.com/theme/pato.json';
const PATO_UPDATE_CACHE    = 'pato_update_response';
 ...
 * @return bool
 */
function pato_updates_enabled() {
	/**
	 * Filters whether Pato checks for updates.
	 *
	 ...
	 */
	return (bool) apply_filters( 'pato_check_for_updates', true );
}
 ...
 * @return array
 */
function pato_update_payload() {
	$payload = array(
		'theme'      => 'pato',
		'version'    => PATO_VERSION,
	 ...
	 */
	return apply_filters( 'pato_update_payload', $payload );
}
 ...
 * @return array|null
 */
function pato_fetch_update() {
	if ( ! pato_updates_enabled() ) {
		return null;
	}

	$cached = get_site_transient( PATO_UPDATE_CACHE );
	...
	$response = wp_remote_get(
		add_query_arg( pato_update_payload(), PATO_UPDATE_ENDPOINT ),
		array(
			'timeout'    => 8,
			'user-agent' => pato_update_user_agent(),
		)
	);

	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		set_site_transient( PATO_UPDATE_CACHE, array(), HOUR_IN_SECONDS );
		return null;
	}
	...
	set_site_transient( PATO_UPDATE_CACHE, $data, 12 * HOUR_IN_SECONDS );
 ...
 * @return array|false
 */
function pato_check_update( $update, $theme_data, $theme_stylesheet ) {
	if ( $update || 'pato' !== $theme_stylesheet ) {
		return $update;
	}

	$release = pato_fetch_update();

	if ( ! $release || version_compare( $release['version'], PATO_VERSION, '<=' ) ) {
		return $update;
	}

	return array(
		'id'           => PATO_UPDATE_ENDPOINT,
		'theme'        => $theme_stylesheet,
		'version'      => $release['version'],
		'url'          => isset( $release['url'] ) ? $release['url'] : 'https://colorlib.com/wp/themes/pato/',
...
add_filter( 'update_themes_' . PATO_UPDATE_HOST, 'pato_check_update', 10, 3 );

/**
 * Forget the cached response when someone asks WordPress to check again.
 */
function pato_flush_update_cache() {
	delete_site_transient( PATO_UPDATE_CACHE );
}
add_action( 'upgrader_process_complete', 'pato_flush_update_cache' );
add_action( 'after_switch_theme', 'pato_flush_update_cache' );
 ...
 * their server.
 */
function pato_updates_notice() {
	$screen = get_current_screen();

	if ( ! $screen || 'appearance_page_pato-starter-sites' !== $screen->id ) {
		return;
	}
	...
	$payload = pato_update_payload();

	// Pato ships no companion plugin, so this speaks only for the theme. If one
	...
	$note = __( 'Pato checks %1$s for updates twice a day and sends %2$s. No personal data and no site name is sent, and the identifier is a one-way hash. Add the %3$s filter to switch it off.', 'pato' );
	?>
	<p class="description pato-updates-note">
		<?php
		printf(
			esc_html( $note ),
			'<code>' . esc_html( PATO_UPDATE_HOST ) . '</code>',
			'<code>' . esc_html( implode( ', ', array_keys( $payload ) ) ) . '</code>',
			'<code>pato_check_for_updates</code>'
		);
		?>
	</p>
	<style>.pato-updates-note { max-width: 70ch; margin-top: 18px; }</style>
	<?php
}
add_action( 'admin_footer', 'pato_updates_notice' );
 ...
 * @return string
 */
function pato_update_user_agent() {
	return 'Pato/' . PATO_VERSION . '; WordPress/' . get_bloginfo( 'version' );
}
 ...
 * @return array
 */
function pato_update_request_args( $args, $url ) {
	if ( 'updates.colorlib.com' === wp_parse_url( $url, PHP_URL_HOST ) ) {
		$args['user-agent'] = pato_update_user_agent();
	}
	return $args;
}
add_filter( 'http_request_args', 'pato_update_request_args', 10, 2 );
