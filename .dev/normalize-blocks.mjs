...
// PATO_PATTERNS says where the patterns actually are in that case.
const patterns = process.env.PATO_PATTERNS
	? resolve( process.env.PATO_PATTERNS )
...
// `<?php ... ?>` -> `PATOPHP0`, and back again after the round trip.
function stash( content ) {
	...
	const stashed = content.replace( /<\?php[\s\S]*?\?>/g, ( match ) => {
		...
		return 'PATOPHP' + seen.get( match ) + 'X';
	} );
...
function restore( content, snippets ) {
	return content.replace( /PATOPHP(\d+)X/g, ( _, i ) => snippets[ Number( i ) ] );
}
